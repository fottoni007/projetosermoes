...
function exportCsv(profiles: PastorProfile[]) {
  const headers = [
    "Nome", "Igreja", "Morada", "Anos Pastorado", "Tipo",
...
export default function AdminPastorsTable({ profiles }: Props) {
  ...
  return (
    <div>
      ...
      {profiles.length === 0 ? (
        <div className="rounded-lg border border-dashed border-ink/20 bg-white p-8 text-center text-sm text-ink/50">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-ink/10 bg-mist/50">
                  <tr>
                    {["Nome", "Igreja", "Tipo", "Contacto", "Estado", "Auto-Aprova.", ""].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-ink/50">
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {profiles.map((p) => (
                    <tr key={p.id} className="hover:bg-mist/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-ink/70">
                        ...
                        <div className="text-xs text-ink/45">{p.address}</div>
                      </td>
                      <td className="px-4 py-3 text-ink/70">
                      ...
                      <td className="px-4 py-3">
                        {p.status === "approved" ? (
                          <button
                            ...
                            onClick={() =>
                              startTransition(() =>
                                toggleAutoApproval(p.id, !p.auto_approve_revoked)
                              )
                            }
                            className="inline-flex items-center gap-1.5 text-xs font-medium transition hover:opacity-80 disabled:opacity-40"
                            title={p.auto_approve_revoked ? "Activar auto-aprovação" : "Revogar auto-aprovação"}
                          >
                            {p.auto_approve_revoked ? (
                              <><ToggleLeft size={18} className="text-ink/40" /> Revogada</>
                            ) : (
                      ...
                      <td className="px-4 py-3">
                        {(p.instagram_url || p.facebook_url || p.youtube_url) && (
                          <div className="flex gap-2 text-xs text-ink/45">
                            {p.instagram_url && <a href={p.instagram_url} target="_blank" className="hover:text-olive" rel="noreferrer">IG</a>}
                            {p.facebook_url && <a href={p.facebook_url} target="_blank" className="hover:text-olive" rel="noreferrer">FB</a>}
                            {p.youtube_url && <a href={p.youtube_url} target="_blank" className="hover:text-olive" rel="noreferrer">YT</a>}
                          </div>
                        )}
                      </td>
                    </tr>
              ...
              <div key={p.id} className="rounded-xl border border-ink/10 bg-white p-4 shadow-soft">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-sm text-ink/65">{p.church_name}</p>
                  </div>
                ...
                <dl className="mt-3 grid grid-cols-2 gap-2 text-xs text-ink/60">
                  <div><dt className="font-medium text-ink/40">Tipo</dt><dd>{p.pastor_type === "senior" ? "Sénior" : "Colaborador"}</dd></div>
                  <div><dt className="font-medium text-ink/40">Anos</dt><dd>{p.years_of_ministry}</dd></div>
                  <div className="col-span-2"><dt className="font-medium text-ink/40">Email</dt><dd>{p.contact_email}</dd></div>
                  <div className="col-span-2"><dt className="font-medium text-ink/40">Telefone</dt><dd>{p.phone_country_code} {p.phone_number}</dd></div>
                </dl>
                {p.status === "approved" && (
                  <div className="mt-3 border-t border-ink/5 pt-3">
                    ...
                    >
                      {p.auto_approve_revoked ? (
                        <><ToggleLeft size={16} className="text-ink/40" /> Auto-aprovação revogada</>
                      ) : (
