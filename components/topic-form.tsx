  ...
  return (
    <form action={formAction} className="space-y-4 rounded-xl border border-ink/10 bg-white p-5 shadow-soft sm:p-6">
      <label className="block">
        ...
        <input name="title" required minLength={4} className="mt-2 w-full rounded-md border border-ink/15 bg-white px-3 py-2 text-sm outline-none transition focus:border-olive focus:ring-4 focus:ring-olive/10" placeholder="Ex.: Como estruturar uma introdução cativante?" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-ink">Conteúdo *</span>
        <textarea name="body" required minLength={10} className="mt-2 min-h-44 w-full rounded-md border border-ink/15 bg-white px-3 py-2 text-sm leading-6 outline-none transition focus:border-olive focus:ring-4 focus:ring-olive/10" placeholder="Partilha a tua questão ou reflexão sobre a arte da pregação..." />
      </label>
