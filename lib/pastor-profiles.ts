...
export async function savePastorProfile(
  ...
  if (existing && existing.status === "approved") {
    return { message: "O teu perfil já foi aprovado e não pode ser alterado." };
  }
  ...
  return {
    message: existing
      ? "Perfil actualizado. Aguarda a aprovação."
      : "Perfil submetido com sucesso. Aguarda a aprovação.",
    success: true
  };
}
...
export async function rejectPastorProfile(id: string): Promise<void> {
  ...
  await supabase
    .from("pastor_profiles")
    .update({ status: "rejected", rejection_reason: rejectionMessage })
    .eq("id", id);
